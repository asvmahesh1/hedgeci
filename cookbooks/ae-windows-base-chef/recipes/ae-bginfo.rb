...
windows_zipfile 'bginfozip' do
  ...
  action :unzip
  not_if { ::File.exist?("#{node['SysinternalsBginfo']['bginfo_installation_directory']}/bginfo.exe") }
end
...
registry_key 'HKEY_CURRENT_USER\\Software\\Sysinternals\\BgInfo' do
  values [{
    :name => 'Wallpaper',
    :type => :string ,
